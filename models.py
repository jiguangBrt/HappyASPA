...
class User(UserMixin, db.Model):
    ...
    # relationships
    vocab_progress = db.relationship(
        "UserVocabularyProgress", backref="user", lazy=True
    )
    flashcard_progress = db.relationship(
        "UserFlashcardProgress", backref="user", lazy=True
    )
    ...
    forum_favorites = db.relationship(
        "ForumFavorite", backref="user", lazy=True
    )  # <--- 新增：收藏关联
    listening_progress = db.relationship(
        "UserListeningProgress", backref="user", lazy=True
    )
    writing_submissions = db.relationship(
        "UserWritingSubmission", backref="user", lazy=True
    )
...
class ForumPost(db.Model):
    ...
    comments = db.relationship(
        "ForumComment", backref="post", lazy=True, cascade="all, delete-orphan"
    )
    likes = db.relationship(
        "ForumLike", backref="post", lazy=True, cascade="all, delete-orphan"
    )
    favorites = db.relationship(
        "ForumFavorite", backref="post", lazy=True, cascade="all, delete-orphan"
    )  # <--- 新增：收藏关联
    ...
    def is_liked_by(self, user):
        ...
        return (
            ForumLike.query.filter_by(post_id=self.id, user_id=user.id).first()
            is not None
        )
    ...
    def is_favorited_by(self, user):
        ...
        return (
            ForumFavorite.query.filter_by(post_id=self.id, user_id=user.id).first()
            is not None
        )
...
class UserListeningProgress(db.Model):
    ...
    exercise_id = db.Column(
        db.Integer, db.ForeignKey("listening_exercises.id"), nullable=False
    )
    ...
    two_thirds_count = db.Column(
        db.Integer, default=0
    )  # count of finishing exercises (Which reachs 2/3 progress of the whole exercise)
...
class WritingExercise(db.Model):
    ...
    submissions = db.relationship(
        "UserWritingSubmission", backref="exercise", lazy=True
    )
...
class UserWritingSubmission(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    exercise_id = db.Column(
        db.Integer, db.ForeignKey("writing_exercises.id"), nullable=False
    )
    content = db.Column(db.Text, nullable=False)
...
class SpeakingExercise(db.Model):
    ...
    )  # 如：interview/lecture/discussion
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    submissions = db.relationship(
        "UserSpeakingSubmission", backref="exercise", lazy=True
    )
...
class UserSpeakingSubmission(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    exercise_id = db.Column(
        db.Integer, db.ForeignKey("speaking_exercises.id"), nullable=False
    )
...
class UserScheduleItem(db.Model):
    ...
    )  # listening / speaking / vocabulary / custom
    title = db.Column(db.String(200), nullable=False)
