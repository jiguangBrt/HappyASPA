...
def index():
    ...
    # 获取用户的所有原始提交记录
    raw_submissions = UserSpeakingSubmission.query.filter_by(
...
def new_exercise():
    if request.method == "POST":
        ...
        if not title or not prompt:
            ...
            return redirect(
                url_for("speaking.exercise_detail", exercise_id=exercise.id)
            )
...
def upload_audio():
    try:
        ...
        if file.content_length and file.content_length > MAX_FILE_SIZE:
            return (
                jsonify({"status": "error", "message": "File too large (max 50MB)"}),
                413,
            )
        ...
        if file and allowed_file(file.filename):
            ...
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "File type not allowed (only mp3/wav/ogg/webm)",
                    }
                ),
                400,
            )
        ...
        return (
            jsonify({"status": "error", "message": "Server error, please try again"}),
            500,
        )
...
def delete_submission(sub_id):
    submission = UserSpeakingSubmission.query.filter_by(
    ...
    filepath = os.path.join(
        current_app.config["UPLOAD_FOLDER"], submission.audio_filename
    )
...
def exercise_detail(exercise_id):
    ...
    submissions = (
        db.session.query(UserSpeakingSubmission, User)
        .join(User, UserSpeakingSubmission.user_id == User.id)
        .filter(UserSpeakingSubmission.exercise_id == exercise_id)
        .order_by(UserSpeakingSubmission.submitted_at.desc())
        .all()
    )
    ...
    for sub, user in submissions:
        audio_path = os.path.join(
            current_app.config["UPLOAD_FOLDER"], sub.audio_filename
        )
    ...
    return render_template(
        "speaking/detail.html", exercise=exercise, submissions=submission_list
    )
...
def delete_exercise(exercise_id):
    ...
    try:
        submissions = UserSpeakingSubmission.query.filter_by(
